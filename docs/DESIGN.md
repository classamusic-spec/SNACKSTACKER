...
- **Backdrop:** infinite seamless gradient sweep (a photographer's cyclorama), tinted per theme, with a soft vignette. No skyboxes, no horizons, no clutter above the scene. The tower is the hero.
- **Every theme is a place.** The plate rests on a real table, and behind it is a scene that says where you are: a backyard barbecue, a hinoki sushi counter under blossom, a string-lit patio, a piazza terrace. Built once per run via `ThemeDef.environment`. It is depth and context, never competition — everything sits at or below the table plane, and it must read from any camera angle because the home screen orbits a full turn.
- **The clearance cylinder.** Portrait gives only ~22° of horizontal field, so at the tower's distance the visible slot is about four units wide and the food fills more than half of it. Every prop must therefore sit below the table plane, or beyond r 4.6 with its top under `tableTopY + 1.5`, or beyond r 9.5 where it reads as background. Express it in cylindrical coordinates and it is yaw-independent by construction, which is what makes it survive the home screen's full turntable rotation.
- **…but the cylinder is necessary, not sufficient.** The camera pitches down, so a distant object projects *higher* on screen than a near one at the same height. A cypress obeying the cylinder at r 10 still read as a spike growing out of the pizza; it had to go out to r 13.5–17.5 before distance put it behind the tower's shoulder instead of above it. Check the frame, not only the maths.
- **Grade against the palette's key, not by habit.** Exposure and bloom run *inversely* to how bright a palette is. The two high-key themes (Candy Stack, Breakfast Rush) take the lowest exposure and bloom of the six; the dark one (Sushi Tower) takes the most. Reversing this is what washed both bright themes out to white.
